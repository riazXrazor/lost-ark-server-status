...
export default class Check extends Command {
  ...
  public async run(): Promise<void> {
    ...
    const res = await got('https://www.playlostark.com/en-gb/support/server-status');
    const $ = load(res.body)

    const regions: any = {}

    $('.ags-ServerStatus-content-tabs').find('a div').each((i, e: any) => regions[e.firstChild.nodeValue.trim().trim('\n')] = [])

    const regionsArray = Object.keys(regions)

    $('.ags-ServerStatus-content-responses').find('.ags-js-serverResponse').each((pindex, el: any) => {
      ...
      el.children.forEach((element: any) => {
        ...
        if (element.type == 'tag') {
          element.children.forEach((innerEl: any) => {
            ...
            if (innerEl.type == 'tag' && innerEl.attribs.class.includes('ags-ServerStatus-content-responses-response-server-name') && innerEl.firstChild.nodeValue.trim()) {
              servername = innerEl.firstChild.nodeValue.trim()
            }
            if (innerEl.type == 'tag' && innerEl.attribs.class.includes('ags-ServerStatus-content-responses-response-server-status-wrapper')) {
              serverstatus = innerEl.children[1].attribs.class.split('-').pop()
            }
